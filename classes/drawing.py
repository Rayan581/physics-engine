...
class DrawingTool:
    ...
    def _prev_rect(self, surface, cam, wm):
        ...
        g.fill((*DRAW_GHOST_COLOR, 40))
        surface.blit(g, (int(x),int(y)))
        pygame.draw.rect(surface, DRAW_OUTLINE_COLOR, (int(x),int(y),int(w),int(h)), 2)
    ...
    def _prev_text(self, surface, cam, wm):
        ...
        pygame.draw.line(surface, DRAW_OUTLINE_COLOR, (x - 8, y - 10), (x + 8, y - 10), 2)
        pygame.draw.line(surface, DRAW_OUTLINE_COLOR, (x, y - 10), (x, y + 10), 2)
    ...
    def _prev_circle(self, surface, cam, wm):
        ...
        pygame.draw.circle(g, (*DRAW_GHOST_COLOR, 40), (r,r), r)
        surface.blit(g, (int(sc[0])-r, int(sc[1])-r))
        pygame.draw.circle(surface, DRAW_OUTLINE_COLOR, (int(sc[0]),int(sc[1])), r, 2)
        pygame.draw.line(surface, DRAW_OUTLINE_COLOR, (int(sc[0]),int(sc[1])), (int(sm[0]),int(sm[1])), 1)
    ...
    def _prev_polygon(self, surface, cam, wm):
        ...
        if len(all_s) >= 2:
            pygame.draw.lines(surface, DRAW_OUTLINE_COLOR, False, all_s, 2)
        if len(all_s) >= 3:
            g = pygame.Surface(surface.get_size(), pygame.SRCALPHA)
            pygame.draw.polygon(g, (*DRAW_GHOST_COLOR, 30), all_s)
            surface.blit(g, (0,0))
        if len(self._points) >= 2:
            pygame.draw.line(surface, DRAW_OUTLINE_COLOR, all_s[-1], all_s[0], 1)
        if self._points:
            near = self._near_first(wm, cam)
            first_s = (int(x) for x in cam.w2s(*self._points[0]))
            ring_r = POLYGON_CLOSE_RADIUS
            pygame.draw.circle(surface,
                               DRAW_CLOSE_RING_NEAR if near else DRAW_CLOSE_RING_FAR,
    ...
    def _prev_motor(self, surface, cam, wm):
        sm = cam.w2s(*wm)
        pygame.draw.circle(surface, DRAW_MOTOR_PREVIEW, (int(sm[0]), int(sm[1])), max(4, int(6*cam.zoom)), 2)
        pygame.draw.circle(surface, DRAW_MOTOR_PREVIEW, (int(sm[0]), int(sm[1])), max(1, int(2*cam.zoom)))
